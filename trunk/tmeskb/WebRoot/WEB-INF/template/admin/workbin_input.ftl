						...
						<div class="col-xs-12">
							...
							<form class="form-horizontal validate" id="searchform1" name="searchform1"  method="post"
								...
								<div class="operateBar">
                                    ...
                                    <label class="col-sm-1 col-md-offset-1"style="text-align:right">交货单号:*</label>
										<div class="col-sm-3">
											<input type="text" name="bktxt"
												class="input input-sm form-control" value="${(bktxt)! }" id="bktxt">
										</div>
										<a id="btn_creat"
													...
													<tr>
														...
														<#list list_cs as list>
															<tr>
																<td>
																	${(list.werks)! }
																	<input type="hidden" name="list_cs[${num}].werks" value="${(list.werks)! }" />
																</td>
																<td>
																	${(list.bktxt)! }
																	<input type="hidden" name="list_cs[${num}].bktxt" value="${(list.bktxt)! }" />
																</td>
																<td>
																	${(list.shipmentsDate)! }
																	<input type="hidden" name="list_cs[${num}].shipmentsDate" value="${(list.shipmentsDate)! }" />
																</td>
																<td>
																	${(list.jslgort)! }
																	<input type="hidden" name="list_cs[${num}].jslgort" value="${(list.jslgort)! }" />
																</td>
																<td>
																	${(list.matnr)! }
																...
																<td>
																	${(list.matnrdesc)! }
																	<input type="hidden" name="list_cs[${num}].matnrdesc" value="${(list.matnrdesc)! }" />
																</td>
																<td>
																	${(list.wscount)! }
																	<input type="hidden" name="list_cs[${num}].wscount" value="${(list.wscount)! }" />
																	<input type="hidden" name="list_cs[${num}].jslgortDesc" value="${(list.jslgortDesc)! }" />
																</td>
															</tr>
															<#assign num=num+1/>
														</#list>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#btn_save").click(function(){
			...
			var url="workbin!creditsave.action";	
			save_event(url);
		});
