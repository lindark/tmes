...
public class WorkingInoutDaoImpl extends BaseDaoImpl<WorkingInout, String> implements
	...
	public List<String[]> sumAmount(String unit,String aufnr,String start,String end,List<String[]> processList) {
		...
		if(processList!=null){
			...
			for(int i=1;i<=k;i++){
				Object[] arry = processList.get(i-1);
				String karry = arry[0].toString();
				sql2 = "(SELECT WIT.ID,PAMJS.BMTJS"+i+",PAMJS.RMTJS"+i+" FROM WORKINGINOUT WIT,"+"(SELECT ROUND(SUM(SPPS.BOMAMOUNT),3) BMTJS"+i+",ROUND(SUM(SPPS.REPAIRAMOUNT),3) RMTJS"+i+",SPPS.BOMCODE,SPPS.MATNR,SPPS.PROCESSID,SPPS.WORKINGBILL_ID FROM (SELECT PHOS.BOMCODE,PHOS.BOMAMOUNT,PHOS.REPAIRAMOUNT,PPS.WORKINGBILL_ID,PPS.PROCESSID,PPS.MATNR FROM PROCESSHANDOVERSON PHOS,"
						+ " (SELECT PHO.ID,PHO.WORKINGBILL_ID,PHO.PROCESSID,PHO.MATNR FROM PROCESSHANDOVER PHO,PROCESSHANDOVERTOP PHOT WHERE PHO.ISDEL='N' AND PHO.WORKINGBILL_ID IS NOT NULL AND PHO.PROCESSID IS NOT NULL AND PHO.PROCESSHANDOVERTOP_ID=PHOT.ID  AND PHOT.STATE='2') PPS "
						+ " WHERE PHOS.PROCESSHANDOVER_ID = PPS.ID) SPPS GROUP BY SPPS.BOMCODE,SPPS.WORKINGBILL_ID,SPPS.PROCESSID,SPPS.MATNR)PAMJS"+" WHERE  WIT.MATERIALCODE=PAMJS.BOMCODE AND WIT.WORKINGBILL_ID=PAMJS.WORKINGBILL_ID AND PAMJS.PROCESSID='"+karry+"') PRCJS"+i;
			...
			String sql3 = "SELECT JSSJ2.*,DECODE(SUBSTR(JSSJ2.MATERIALCODE,0,1),'5',(JSSJ2.RKS+JSSJ2.LTZCJX+JSSJ2.LTYCJX-JSSJ2.LTZCJS-JSSJ2.LTYCJS)*JSSJ2.YL+JSSJ2.FXFHMT-JSSJ2.FXSHMT,JSSJ2.RKS*JSSJ2.YL) CCZSL,TO_CHAR(DECODE(SIGN(JSSJ2.JHS),1,ROUND((JSSJ2.SCS/JSSJ2.JHS),2),0),'fm999999990.999999999')||'%' JHDCL "
					+ " FROM "
			...
			String sqlhj = " )ZZD LEFT JOIN (SELECT DECODE(SIGN(ROUND(SUM(PHJS.ACTUALHOMOUNT), 3)),1,ROUND(SUM(PHJS.ACTUALHOMOUNT), 3),0) ZCJS, DECODE(SIGN(ROUND(SUM(PHJS.UNHOMOUNT), 3)),1,ROUND(SUM(PHJS.UNHOMOUNT), 3),0) YCJS,PHJS.AFTERWORKINGBILLCODE PHWCJS FROM PROCESSHANDOVER PHJS,PROCESSHANDOVERTOP PHTJS WHERE  PHJS.PROCESSHANDOVERTOP_ID = PHTJS.ID AND PHJS.ISDEL = 'N' AND PHTJS. TYPE = '零头数交接' AND PHTJS.STATE = '2' GROUP BY PHJS.AFTERWORKINGBILLCODE)JSLT ON ZZD.WORKINGBILLCODE=JSLT.PHWCJS  "
					...
					+ " LEFT JOIN (SELECT YL1.WKID WKID1,YL1.MATERIALCODE MCD,ROUND(YL1.DWYL,2) DL FROM (SELECT WKI1.ID WKID,WKI1.MATERIALCODE,DECODE(SIGN(WKI1.PLANCOUNT),1,(SUM(BBM.MAM)/WKI1.PLANCOUNT),0)DWYL  FROM (SELECT WIT3.ID,WB1.WORKCENTER,WB1.FUZHUREN,WB1.ZHUREN,WB1.PRODUCTDATE,WB1.SHIFT,WB1.AUFNR,WB1.MATNR,WB1.MAKTX,WIT3.MATERIALCODE,WIT3.MATERIALNAME,WB1.WORKINGBILLCODE,WB1.PLANCOUNT FROM WORKINGINOUT WIT3 ,(SELECT WB2.ID,WB2.WORKCENTER,WB2.WORKINGBILLCODE,WB2.PLANCOUNT,TM.TEAMNAME,WB2.FUZHUREN,WB2.ZHUREN,WB2.PRODUCTDATE,WB2.SHIFT,WB2.AUFNR,WB2.MATNR,WB2.MAKTX FROM WORKINGBILL WB2 LEFT JOIN TEAM TM ON WB2.TEAM_ID=TM.ID)WB1 WHERE WIT3.WORKINGBILL_ID=WB1.ID) WKI1, (SELECT B.MATERIALCODE,B.SHIFT,ORD.AUFNR,MAX(B.VERSION) BV,SUM(DECODE(SIGN(B.MATERIALAMOUNT),1,B.MATERIALAMOUNT,0))MAM FROM BOM B,ORDERS ORD WHERE B.ORDERS_ID = ORD.ID AND B.ISDEL = 'N' GROUP BY B.MATERIALCODE,B.SHIFT,ORD.AUFNR ) BBM "
					...
					+ " union "
					+ " SELECT YL2.WKID,YL2.MATERIALCODE,YL2.DWYL FROM (SELECT WKI2.ID WKID,WKI2.MATERIALCODE,(DECODE(SIGN(SUM(USIN.CONVERSATIONRATIO)),1,ROUND(SUM(1/USIN.CONVERSATIONRATIO),4),0)) DWYL  FROM (SELECT WIT3.ID,WB3.WORKCENTER,WB3.FUZHUREN,WB3.ZHUREN,WB3.PRODUCTDATE,WB3.SHIFT,WB3.AUFNR,WB3.MATNR,WB3.MAKTX,WIT3.MATERIALCODE,WIT3.MATERIALNAME,WB3.WORKINGBILLCODE,WB3.PLANCOUNT FROM WORKINGINOUT WIT3 ,(SELECT WB2.ID,WB2.WORKCENTER,WB2.WORKINGBILLCODE,WB2.PLANCOUNT,TM.TEAMNAME,WB2.FUZHUREN,WB2.ZHUREN,WB2.PRODUCTDATE,WB2.SHIFT,WB2.AUFNR,WB2.MATNR,WB2.MAKTX FROM WORKINGBILL WB2 LEFT JOIN TEAM TM ON WB2.TEAM_ID=TM.ID)WB3 WHERE WIT3.WORKINGBILL_ID=WB3.ID) WKI2, "
			...
			if(!"".equals(selectsum)){
				...
				sql5 = "SELECT TO_CHAR(DECODE(JSSJ4.WORKCENTER,NULL,' ',JSSJ4.WORKCENTER))||' ',TO_CHAR(DECODE(JSSJ4.TEAMNAME,NULL,' ',JSSJ4.TEAMNAME))||' ',TO_CHAR(DECODE(JSSJ4.ZHUREN,NULL,' ',JSSJ4.ZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.FUZHUREN,NULL,' ',JSSJ4.FUZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.PRODUCTDATE,NULL,' ',JSSJ4.PRODUCTDATE))||' ',TO_CHAR(DECODE(JSSJ4.SHIFT,NULL,' ',JSSJ4.SHIFT))||' ',TO_CHAR(DECODE(JSSJ4.AUFNR,NULL,' ',JSSJ4.AUFNR))||' ',TO_CHAR(DECODE(JSSJ4.MATNR,NULL,' ',JSSJ4.MATNR))||' ',TO_CHAR(DECODE(JSSJ4.MAKTX,NULL,' ',JSSJ4.MAKTX))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALCODE,NULL,' ',JSSJ4.MATERIALCODE))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALNAME,NULL,' ',JSSJ4.MATERIALNAME))||' ',TO_CHAR(NVL(JSSJ4.YL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LYS,'0'))||' ',"+strbt+" TO_CHAR(NVL(JSSJ4.BFS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.RKS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXFHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXSHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.SCS,'0'))||' ',TO_CHAR(NVL('','0'))||' ',TO_CHAR(NVL(JSSJ4.TRZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.CCZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.SLCY,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHDCL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JJZT,'0'))||' ',TO_CHAR(NVL(JSSJ4.BGS,'0'))||' ',TO_CHAR(NVL(JSSJ4.JYCY,'0'))||' ',TO_CHAR(NVL(YCQ.YLS,'0'))||' ' YCQRS,TO_CHAR(NVL(SJCQ.ALS,'0'))||' ' SJCQRS"
						+ " FROM ("+sql4+")JSSJ4 "
						+ " LEFT JOIN "+sqlselect+" ON JSSJ4.WITID=PRSTJX"+k+".ID "
						+"  LEFT JOIN (SELECT PRODUCTDATE,CLASSTIME,TEAM_ID,COUNT(*)YLS FROM TEMPKAOQIN GROUP BY PRODUCTDATE,CLASSTIME,TEAM_ID)YCQ ON JSSJ4.TMID=YCQ.TEAM_ID AND JSSJ4.PRODUCTDATE=YCQ.PRODUCTDATE AND SUBSTR(JSSJ4.SHIFT,1,1)=YCQ.CLASSTIME "
				...
				sql4 = "SELECT JSSJ3.*,(JSSJ3.LYS)TRZSL,(JSSJ3.CCZSL-JSSJ3.LYS)SLCY "
						+ "FROM ("+sql3+")JSSJ3";
				sql5 = "SELECT TO_CHAR(DECODE(JSSJ4.WORKCENTER,NULL,' ',JSSJ4.WORKCENTER))||' ',TO_CHAR(DECODE(JSSJ4.TEAMNAME,NULL,' ',JSSJ4.TEAMNAME))||' ',TO_CHAR(DECODE(JSSJ4.ZHUREN,NULL,' ',JSSJ4.ZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.FUZHUREN,NULL,' ',JSSJ4.FUZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.PRODUCTDATE,NULL,' ',JSSJ4.PRODUCTDATE))||' ',TO_CHAR(DECODE(JSSJ4.SHIFT,NULL,' ',JSSJ4.SHIFT))||' ',TO_CHAR(DECODE(JSSJ4.AUFNR,NULL,' ',JSSJ4.AUFNR))||' ',TO_CHAR(DECODE(JSSJ4.MATNR,NULL,' ',JSSJ4.MATNR))||' ',TO_CHAR(DECODE(JSSJ4.MAKTX,NULL,' ',JSSJ4.MAKTX))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALCODE,NULL,' ',JSSJ4.MATERIALCODE))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALNAME,NULL,' ',JSSJ4.MATERIALNAME))||' ',TO_CHAR(NVL(JSSJ4.YL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LYS,'0'))||' ', TO_CHAR(NVL(JSSJ4.BFS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.RKS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXFHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXSHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.SCS,'0'))||' ',TO_CHAR(NVL('','0'))||' ',TO_CHAR(NVL(JSSJ4.TRZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.CCZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.SLCY,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHDCL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JJZT,'0'))||' ',TO_CHAR(NVL(JSSJ4.BGS,'0'))||' ',TO_CHAR(NVL(JSSJ4.JYCY,'0'))||' ',TO_CHAR(NVL(YCQ.YLS,'0'))||' ' YCQRS,TO_CHAR(NVL(SJCQ.ALS,'0'))||' ' SJCQRS"
						+ " FROM ("+sql4+")JSSJ4 "
						+"  LEFT JOIN (SELECT PRODUCTDATE,CLASSTIME,TEAM_ID,COUNT(*)YLS FROM TEMPKAOQIN GROUP BY PRODUCTDATE,CLASSTIME,TEAM_ID)YCQ ON JSSJ4.TMID=YCQ.TEAM_ID AND JSSJ4.PRODUCTDATE=YCQ.PRODUCTDATE AND SUBSTR(JSSJ4.SHIFT,1,1)=YCQ.CLASSTIME "
						+ " LEFT JOIN (SELECT PRODUCTDATE,CLASSTIME,TEAM_ID,COUNT(*)ALS FROM TEMPKAOQIN WHERE WORKSTATE='2' GROUP BY PRODUCTDATE,CLASSTIME,TEAM_ID)SJCQ ON JSSJ4.TMID=SJCQ.TEAM_ID AND JSSJ4.PRODUCTDATE=SJCQ.PRODUCTDATE AND SUBSTR(JSSJ4.SHIFT,1,1)=SJCQ.CLASSTIME ORDER BY JSSJ4.WORKCENTER,JSSJ4.TEAMNAME,JSSJ4.PRODUCTDATE,JSSJ4.SHIFT,JSSJ4.AUFNR,JSSJ4.MATNR";
	...
	public List<String[]> sumAmountSY(HashMap<String, List<String>> map) {
		...
		if(!"".equals(selectsum)){
			sql = "SELECT TO_CHAR(JSSJ1.WORKINGBILLCODE) WBCD,TO_CHAR(SUM(ROUND(JSSJ1.LYS+DECODE(SIGN(GXJJ1.SAD),1,GXJJ1.SAD,0),2)-DECODE(SUBSTR(JSSJ1.MATERIALCODE,0,1),5,ROUND((JSSJ1.RKS+JSSJ1.ZCJX+JSSJ1.YCJX-JSSJ1.ZCJS-YCJS)*JSSJ1.DWYL+JSSJ1.FHMT-JSSJ1.SHMT,2),ROUND(JSSJ1.RKS*JSSJ1.DWYL,2))))SLCY FROM ("+sql+")JSSJ1 "
					+ "LEFT JOIN "
					+ "("+selectsum+")GXJJ1 ON JSSJ1.WITID=GXJJ1.ID GROUP BY JSSJ1.WORKINGBILLCODE";
		}else{
			sql = "SELECT JSSJ1.WORKINGBILLCODE,SUM(JSSJ1.LYS-DECODE(SUBSTR(JSSJ1.MATERIALCODE,0,1),5,ROUND((JSSJ1.RKS+JSSJ1.ZCJX+JSSJ1.YCJX-JSSJ1.ZCJS-YCJS)*JSSJ1.DWYL+JSSJ1.FHMT-JSSJ1.SHMT,2),ROUND(JSSJ1.RKS*JSSJ1.DWYL,2)))SLCY FROM ("+sql+")JSSJ1 GROUP BY JSSJ1.WORKINGBILLCODE";
		}
