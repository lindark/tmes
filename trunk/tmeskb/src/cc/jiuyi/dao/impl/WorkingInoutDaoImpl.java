...
public class WorkingInoutDaoImpl extends BaseDaoImpl<WorkingInout, String> implements
	...
	public List<String[]> sumAmount(String aufnr,String unit,String start,String end,List<String[]> processList) {
		...
		if(processList!=null){
			...
			String sql3 = "SELECT JSSJ2.*,DECODE(SUBSTR(JSSJ2.MATERIALCODE,0,1),'5',JSSJ2.RKS*JSSJ2.YL,(JSSJ2.RKS+JSSJ2.LTZCJX+JSSJ2.LTYCJX-JSSJ2.LTZCJS-JSSJ2.LTYCJS)*JSSJ2.YL+JSSJ2.FXFHMT-JSSJ2.FXSHMT+JSSJ2.BFS) CCZSL,TO_CHAR(DECODE(SIGN(JSSJ2.JHS),1,ROUND((JSSJ2.SCS/JSSJ2.JHS*100),2),0),'fm999999990.999999999')||'%' JHDCL "
					+ " FROM "
					...
					+ "(SELECT ZZD.*,DECODE(SIGN(LYS1.REPMOUNT),1,LYS1.REPMOUNT,0)LYS ,DECODE(SIGN(DWYL6.DL),1,DWYL6.DL,0) YL,DECODE(SIGN(JSLT.ZCJS),1,JSLT.ZCJS,0) LTZCJS,DECODE(SIGN(JSLT.YCJS),1,JSLT.YCJS,0) LTYCJS,DECODE(SIGN(JXLT.ZCJX),1,JXLT.ZCJX,0) LTZCJX,DECODE(SIGN(JXLT.YCJX ),1,JXLT.YCJX ,0) LTYCJX,DECODE(SIGN(FXFH.FHMT),1,FXFH.FHMT ,0) FXFHMT,DECODE(SIGN(FXSH.SHMT),1,FXSH.SHMT ,0) FXSHMT,(ZZD.RKS+DECODE(SIGN(JXLT.ZCJX),1,JXLT.ZCJX,0)-ZZD.BGS-DECODE(SIGN(JSLT.ZCJS),1,JSLT.ZCJS,0)-DECODE(SIGN(FXSH.SHMT),1,FXSH.SHMT,0))JYCY FROM "
					+ "(SELECT ZZDS.WITID,ZZDS.WBID,ZZDS.WORKCENTER,ZZDS.TEAMNAME,ZZDS.FUZHUREN,ZZDS.ZHUREN,ZZDS.MINISTER,ZZDS.DEPUTY,ZZDS.PRODUCTDATE,ZZDS.SHIFT,ZZDS.AUFNR,ZZDS.MATNR,ZZDS.MAKTX,ZZDS.MATERIALCODE,ZZDS.MATERIALNAME,ZZDS.WORKINGBILLCODE,ZZDS.JHS,DECODE(SIGN(EHS.STM),1,EHS.STM,0) RKS, ZZDS.BFS ,ZZDS.BGS,ZZDS.TMID,ZZDS.JJZT,ZZDS.TFXFH,ZZDS.TFXSH FROM "
					+ "(SELECT WIT3.ID WITID,WB1.ID WBID,WB1.WORKCENTER,WB1.TEAMNAME,WB1.FUZHUREN,WB1.ZHUREN,WB1.MINISTER,WB1.DEPUTY,WB1.PRODUCTDATE,WB1.SHIFT,WB1.AUFNR,WB1.MATNR,WB1.MAKTX,WIT3.MATERIALCODE,WIT3.MATERIALNAME,WB1.WORKINGBILLCODE,DECODE(SIGN(WB1.PLANCOUNT),1,WB1.PLANCOUNT,0) JHS,ROUND(DECODE(SIGN(WIT3.SCRAPNUMBER),1,WIT3.SCRAPNUMBER,0),3) BFS ,DECODE(SIGN(WB1.DAILYWORKTOTALAMOUNT),1,WB1.DAILYWORKTOTALAMOUNT,0) BGS,WB1.TMID,DECODE(WB1.ISHAND,'Y','已交接完成','未交接完成') JJZT,DECODE(SIGN(WB1.TOTALREPAIRAMOUNT),1,WB1.TOTALREPAIRAMOUNT,0) TFXFH,DECODE(SIGN(WB1.TOTALREPAIRINAMOUNT),1,WB1.TOTALREPAIRINAMOUNT,0) TFXSH  "
...
//					+ "(SELECT WB2.ID,WB2.WORKCENTER,WB2.WORKINGBILLCODE,WB2.PLANCOUNT,TM.TEAMNAME,WB2.FUZHUREN,WB2.ZHUREN,WB2.MINISTER,WB2.DEPUTY,WB2.PRODUCTDATE,WB2.SHIFT,WB2.AUFNR,WB2.MATNR,WB2.MAKTX,WB2.TOTALSINGLEAMOUNT,WB2.DAILYWORKTOTALAMOUNT,TM.ID TMID,WB2.ISHAND FROM WORKINGBILL WB2 LEFT JOIN TEAM TM ON WB2.TEAM_ID=TM.ID)WB1 WHERE WIT3.WORKINGBILL_ID=WB1.ID AND WB1.PRODUCTDATE BETWEEN '"+start+"' AND '"+end+"' ";
			...
			String sqlhj = " )ZZD "
					...
					+ " (SELECT B.MATERIALCODE,B.SHIFT,ORD.AUFNR,DECODE(SIGN(B.MATERIALAMOUNT),1,B.MATERIALAMOUNT,0)MAM,MAX(B.VERSION) BV FROM BOM B,ORDERS ORD WHERE B.ORDERS_ID = ORD.ID AND B.ISDEL = 'N' GROUP BY B.MATERIALCODE,B.SHIFT,ORD.AUFNR,B.MATERIALAMOUNT )CYL2 "
			...
			String strbt = "";
			if(!"".equals(selectsum)){
				sql4 = "SELECT JSSJ3.*,GXSJ.SAD,(GXSJ.SAD+JSSJ3.LYS)TRZSL,(JSSJ3.CCZSL-GXSJ.SAD-JSSJ3.LYS)SLCY "
				...
				sql5 = "SELECT TO_CHAR(DECODE(JSSJ4.WORKCENTER,NULL,' ',JSSJ4.WORKCENTER))||' ' ,TO_CHAR(DECODE(JSSJ4.TEAMNAME,NULL,' ',JSSJ4.TEAMNAME))||' ',TO_CHAR(DECODE(JSSJ4.ZHUREN,NULL,' ',JSSJ4.ZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.FUZHUREN,NULL,' ',JSSJ4.FUZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.MINISTER,NULL,' ',JSSJ4.MINISTER))||' ',TO_CHAR(DECODE(JSSJ4.DEPUTY,NULL,' ',JSSJ4.DEPUTY))||' ',TO_CHAR(DECODE(JSSJ4.PRODUCTDATE,NULL,' ',JSSJ4.PRODUCTDATE))||' ',TO_CHAR(DECODE(JSSJ4.SHIFT,NULL,' ',JSSJ4.SHIFT))||' ',TO_CHAR(DECODE(JSSJ4.AUFNR,NULL,' ',JSSJ4.AUFNR))||' ',TO_CHAR(DECODE(JSSJ4.MATNR,NULL,' ',JSSJ4.MATNR))||' ',TO_CHAR(DECODE(JSSJ4.MAKTX,NULL,' ',JSSJ4.MAKTX))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALCODE,NULL,' ',JSSJ4.MATERIALCODE))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALNAME,NULL,' ',JSSJ4.MATERIALNAME))||' ',TO_CHAR(NVL(JSSJ4.YL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LYS,'0'))||' ',"+strbt+" TO_CHAR(NVL(JSSJ4.BFS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.RKS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXFHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXSHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.SCS,'0'))||' ',TO_CHAR(NVL('','0'))||' ',TO_CHAR(NVL(JSSJ4.TRZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.CCZSL,'0'))||' ',TO_CHAR(NVL(ABS(JSSJ4.SLCY),'0'))||' ',TO_CHAR(NVL(JSSJ4.JHDCL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JJZT,'0'))||' ',TO_CHAR(NVL(JSSJ4.BGS,'0'))||' ',TO_CHAR(NVL(JSSJ4.JYCY,'0'))||' ',TO_CHAR(NVL(YCQ.YLS,'0'))||' ' YCQRS,TO_CHAR(NVL(SJCQ.ALS,'0'))||' ' SJCQRS,TO_CHAR(NVL(YCQ.WH,'0'))||' ' YCQWH,TO_CHAR(NVL(RKSL.RKS,'0'))||' ' RKS1,TO_CHAR(NVL(JSSJ4.RKS,0)-NVL(RKSL.RKS,0))||' ' PC, (NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0)+NVL(JSSJ4.TFXFH,0)-NVL(JSSJ4.TFXSH,0))||' ' SCSRK ,(DECODE(SUBSTR(JSSJ4.MATERIALCODE,0,1),'5',NVL(RKSL.RKS,0)*NVL(JSSJ4.YL,0),(NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0))*NVL(JSSJ4.YL,0)+NVL(JSSJ4.FXFHMT,0)-NVL(JSSJ4.FXSHMT,0)+NVL(JSSJ4.BFS,0)))||' ' CCZSLRK,(((NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0))*NVL(JSSJ4.YL,0)+NVL(JSSJ4.FXFHMT,0)-NVL(JSSJ4.FXSHMT,0)+NVL(JSSJ4.BFS,0))-NVL(JSSJ4.SAD,0)-NVL(JSSJ4.LYS,0))||' ' SLCYRK"
						+ " FROM ("+sql4+")JSSJ4 "
				...
				sql4 = "SELECT JSSJ3.*,(JSSJ3.LYS)TRZSL,(JSSJ3.CCZSL-JSSJ3.LYS)SLCY "
						+ "FROM ("+sql3+")JSSJ3";
				sql5 = "SELECT TO_CHAR(DECODE(JSSJ4.WORKCENTER,NULL,' ',JSSJ4.WORKCENTER))||' ',TO_CHAR(DECODE(JSSJ4.TEAMNAME,NULL,' ',JSSJ4.TEAMNAME))||' ',TO_CHAR(DECODE(JSSJ4.ZHUREN,NULL,' ',JSSJ4.ZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.FUZHUREN,NULL,' ',JSSJ4.FUZHUREN))||' ',TO_CHAR(DECODE(JSSJ4.MINISTER,NULL,' ',JSSJ4.MINISTER))||' ',TO_CHAR(DECODE(JSSJ4.DEPUTY,NULL,' ',JSSJ4.DEPUTY))||' ',TO_CHAR(DECODE(JSSJ4.PRODUCTDATE,NULL,' ',JSSJ4.PRODUCTDATE))||' ',TO_CHAR(DECODE(JSSJ4.SHIFT,NULL,' ',JSSJ4.SHIFT))||' ',TO_CHAR(DECODE(JSSJ4.AUFNR,NULL,' ',JSSJ4.AUFNR))||' ',TO_CHAR(DECODE(JSSJ4.MATNR,NULL,' ',JSSJ4.MATNR))||' ',TO_CHAR(DECODE(JSSJ4.MAKTX,NULL,' ',JSSJ4.MAKTX))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALCODE,NULL,' ',JSSJ4.MATERIALCODE))||' ',TO_CHAR(DECODE(JSSJ4.MATERIALNAME,NULL,' ',JSSJ4.MATERIALNAME))||' ',TO_CHAR(NVL(JSSJ4.YL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JHS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LYS,'0'))||' ', TO_CHAR(NVL(JSSJ4.BFS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJS,'0'))||' ',TO_CHAR(NVL(JSSJ4.RKS,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTZCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.LTYCJX,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXFHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.FXSHMT,'0'))||' ',TO_CHAR(NVL(JSSJ4.SCS,'0'))||' ',TO_CHAR(NVL('','0'))||' ',TO_CHAR(NVL(JSSJ4.TRZSL,'0'))||' ',TO_CHAR(NVL(JSSJ4.CCZSL,'0'))||' ',TO_CHAR(NVL(ABS(JSSJ4.SLCY),'0'))||' ',TO_CHAR(NVL(JSSJ4.JHDCL,'0'))||' ',TO_CHAR(NVL(JSSJ4.JJZT,'0'))||' ',TO_CHAR(NVL(JSSJ4.BGS,'0'))||' ',TO_CHAR(NVL(JSSJ4.JYCY,'0'))||' ',TO_CHAR(NVL(YCQ.YLS,'0'))||' ' YCQRS,TO_CHAR(NVL(SJCQ.ALS,'0'))||' ' SJCQRS,TO_CHAR(NVL(YCQ.WH,'0'))||' ' YCQWH,TO_CHAR(NVL(RKSL.RKS,'0'))||' ' RKS1,TO_CHAR(NVL(JSSJ4.RKS,0)-NVL(RKSL.RKS,0))||' ' PC, (NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0)+NVL(JSSJ4.TFXFH,0)-NVL(JSSJ4.TFXSH,0))||' ' SCSRK ,((NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0))*NVL(JSSJ4.YL,0)+NVL(JSSJ4.FXFHMT,0)-NVL(JSSJ4.FXSHMT,0)+NVL(JSSJ4.BFS,0))||' ' CCZSLRK,(DECODE(SUBSTR(JSSJ4.MATERIALCODE,0,1),'5',NVL(RKSL.RKS,0)*NVL(JSSJ4.YL,0),(NVL(RKSL.RKS,0)+NVL(JSSJ4.LTZCJX,0)+NVL(JSSJ4.LTYCJX,0)-NVL(JSSJ4.LTZCJS,0)-NVL(JSSJ4.LTYCJS,0))*NVL(JSSJ4.YL,0)+NVL(JSSJ4.FXFHMT,0)-NVL(JSSJ4.FXSHMT,0)+NVL(JSSJ4.BFS,0)))||' ' SLCYRK"
						+ " FROM ("+sql4+")JSSJ4 "
						+ " LEFT JOIN (SELECT AUFNR,SGTXT,SUM(DECODE(BWART,'101',MENGE,'102',(-1)*MENGE,0))RKS FROM PRODUCTSTORAGE GROUP BY AUFNR,SGTXT)RKSL ON RKSL.AUFNR= DECODE(SUBSTR(JSSJ4.AUFNR,0,3),'000',JSSJ4.AUFNR,'000'||JSSJ4.AUFNR) AND RKSL.SGTXT=JSSJ4.SHIFT"
						+"  LEFT JOIN (SELECT PRODUCTDATE,CLASSTIME,TEAM_ID,(SELECT DICTVALUE FROM DICT WHERE DICTNAME='workHours' AND DICTKEY=WORKHOURS)WH,COUNT(*)YLS FROM TEMPKAOQIN GROUP BY PRODUCTDATE,CLASSTIME,TEAM_ID,WORKHOURS)YCQ ON JSSJ4.TMID=YCQ.TEAM_ID AND JSSJ4.PRODUCTDATE=YCQ.PRODUCTDATE AND SUBSTR(JSSJ4.SHIFT,2,1)=YCQ.CLASSTIME "
						+ " LEFT JOIN (SELECT PRODUCTDATE,CLASSTIME,TEAM_ID,COUNT(*)ALS FROM TEMPKAOQIN WHERE WORKSTATE='2' GROUP BY PRODUCTDATE,CLASSTIME,TEAM_ID)SJCQ ON JSSJ4.TMID=SJCQ.TEAM_ID AND JSSJ4.PRODUCTDATE=SJCQ.PRODUCTDATE AND SUBSTR(JSSJ4.SHIFT,2,1)=SJCQ.CLASSTIME ORDER BY JSSJ4.WORKCENTER,JSSJ4.TEAMNAME,JSSJ4.PRODUCTDATE,JSSJ4.SHIFT,JSSJ4.AUFNR,JSSJ4.MATNR";
	...
	public List<String[]> sumAmountSY(HashMap<String, List<String>> map) {
		...
		String sql = "SELECT WITS.WITID,WITS.WBID,WITS.WORKINGBILLCODE,WITS.MATERIALCODE,WITS.BFS,EHS.STM RKS,DECODE(SIGN(LYSS.REPMOUNT),1,LYSS.REPMOUNT,0) LYS,DECODE(SIGN( DWYLS.DWYL),1, DWYLS.DWYL,0)DWYL,DECODE(SIGN( LTJS.ZCJS),1, LTJS.ZCJS,0)ZCJS,DECODE(SIGN(LTJS.YCJS),1, LTJS.YCJS,0)YCJS,DECODE(SIGN(LTJX.ZCJX),1,LTJX.ZCJX,0)ZCJX,DECODE(SIGN(LTJX.YCJX),1,LTJX.YCJX,0)YCJX,DECODE(SIGN(FXFH.FHMT),1,FXFH.FHMT,0)FHMT,DECODE(SIGN(FXSH.SHMT),1,FXSH.SHMT,0)SHMT FROM "
				+ "(SELECT WIT.ID WITID,WB.ID WBID,WIT.MATERIALCODE,ROUND(DECODE(SIGN(WIT.SCRAPNUMBER),1,WIT.SCRAPNUMBER,0),3) BFS,WB.WORKINGBILLCODE FROM WORKINGINOUT WIT,(SELECT ID,WORKINGBILLCODE,DECODE(SIGN(TOTALSINGLEAMOUNT),1,TOTALSINGLEAMOUNT,0) RKS FROM WORKINGBILL WHERE ID IN ("+wbid+")) WB WHERE WORKINGBILL_ID IN WB.ID)WITS "
				+ " LEFT JOIN "
				//获取包装数
				...
				+ " (SELECT B.MATERIALCODE,B.SHIFT,OAU.AUFNR,MAX(B.VERSION) BV,DECODE(SIGN(B.MATERIALAMOUNT),1,B.MATERIALAMOUNT,0)MAM "
		...
		if(!"".equals(selectsum)){
			sql = "SELECT TO_CHAR(JSSJ1.WORKINGBILLCODE) WBCD,TO_CHAR(ABS(SUM(ROUND(JSSJ1.LYS+DECODE(SIGN(GXJJ1.SAD),1,GXJJ1.SAD,0),2)-DECODE(SUBSTR(JSSJ1.MATERIALCODE,0,1),'5',ROUND(JSSJ1.RKS*JSSJ1.DWYL,2),ROUND((JSSJ1.RKS+JSSJ1.ZCJX+JSSJ1.YCJX-JSSJ1.ZCJS-YCJS)*JSSJ1.DWYL+JSSJ1.FHMT-JSSJ1.SHMT+JSSJ1.BFS,2)))))SLCY FROM ("+sql+")JSSJ1 "
					+ "LEFT JOIN "
					+ "("+selectsum+")GXJJ1 ON JSSJ1.WITID=GXJJ1.ID GROUP BY JSSJ1.WORKINGBILLCODE";
		}else{
			sql = "SELECT JSSJ1.WORKINGBILLCODE,TO_CHAR(ABS(SUM(JSSJ1.LYS-DECODE(SUBSTR(JSSJ1.MATERIALCODE,0,1),'5',ROUND(JSSJ1.RKS*JSSJ1.DWYL,2),ROUND((JSSJ1.RKS+JSSJ1.ZCJX+JSSJ1.YCJX-JSSJ1.ZCJS-YCJS)*JSSJ1.DWYL+JSSJ1.FHMT-JSSJ1.SHMT+JSSJ1.BFS,2)))))SLCY FROM ("+sql+")JSSJ1 GROUP BY JSSJ1.WORKINGBILLCODE";
		}
