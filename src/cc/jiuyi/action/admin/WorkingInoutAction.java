...
public class WorkingInoutAction extends BaseAdminAction {
	...
	private String[] strlen = {"workingbill.workingBillCode","materialCode","planCount","afteroddamount","afterunoddamount","recipientsAmount","multiple","totalSingleAmount","afterFraction","scrapNumber",
			"totalRepairAmount","totalRepairinAmount","workingbill.productDate","shift","workingbill.aufnr","zjdwyl","dbjyhgs","beforeunoddamount","ychgl","trzsl",
			"cczsl","slcy","jhdcl","workingbill.matnr","workingbill.maktx","materialName","totalAmount","isHand","jycl","workingbill.workcenter",
			"teamCode","teamName","bulkProductMount","needAttendance","actulAttendance","zhuren","fuzhuren","minister","deputy","workHours",
			"storage","offset","rkcy","cchzrk","scsrk","schgs"};
	private String[] lavenlen={"随工单编号","子件编码","计划数量","接/零头","接/零头/返","领用","倍数","包装数",
								"交/零头","报废","返修发货","返修收货","生产日期","班次","生产订单号","用量","生产数/参考","交/零头/返","一次合格率",
								"投入汇总","产出汇总/包装","差异/包装","计划达成率","物料编码","物料描述","组件描述","报工","单据状态","校验差异","单元",
								"班组编码","班组名称","待包装数量","应出勤人数","实出勤人数","主任","副主任","部长","副总","工作小时","入库数","偏差","差异/入库","产出汇总/入库","生产数/入库","生产合格数"};
	public String list(){
		...
		List<String[]> processList = workinginoutservice.findProcess();
 		...
 		if(bodys!=null && bodys.size()>0){
 			...
 			if(processList!=null&&processList.size()>0){
 	 			...
 	 			strList.add("报工");strList.add("检验差异");strList.add("应出勤人数");strList.add("实出勤人数");strList.add("生产时间");strList.add("入库数");strList.add("偏差");strList.add("生产数(入库)");strList.add("生产合格数");strList.add("产出汇总(入库)");strList.add("入库差异");
 				...
 				//,"入库数","偏差"
 	 			strarry = new String[]{
 	 	 				...
 	 	 				,"计划达成率","单据状态","报工","检验差异","应出勤人数","实出勤人数","生产时间","入库数","偏差","生产数(入库)","生产合格数","产出汇总(入库)","入库差异"
 	 	 		};
 	 		}
 		}else{
 			//,"入库数","偏差"
