	...
	$.fn.fcbkcomplete = function(opt) {
		return this.queue(function() {
			...
			function createFCBK() {
				...
				var realWidth = $(holder).outerWidth() || Math.max(options.width + 4, 120);
				complete = $('<div class="facebook-auto">').width(realWidth);
				if (options.complete_text != "") {
					var completeText = options.complete_text;
					complete.append('<div class="default" id="defaultText">' + completeText + '</div>');
				...
				feed = $('<ul id="' + elemid + '_feed"></ul>').width(realWidth);
				// holder.after(complete.prepend(feed));
				complete.prepend(feed).appendTo('body');
				elPrepare();
			}
			...
			function elPrepare() {
				...
				var temp_elem = $('<' + element.get(0).tagName + ' name="' + name + '" id="' + elemid + '" multiple="multiple" title="' + element.attr("title") + '" errMsgId="' + element.attr("errMsgId") + '" class="' + element.get(0).className + ' hidden">').data('cache', {});
			...
			function showComplete() {
				var _holder = ((options.holder && $(options.holder).size() > 0) ? options.holder : holder);//支持自定义弹出基准元素
				complete.add(feed).width($(_holder).outerWidth());
				// if($.browser.msie && $.browser.version < 9){//zl: 解决IE8下，$.position在日程页面left，top算错的bug，值大了两倍
    //                 complete.css('top', 0);
    //                 complete.css('left', 0);
    //             }
				complete.show('fade',function(){
                    if (options.onpopup){
                    	(options.onpopup)(complete)
					}
				});
				complete.position({
					my: 'left top',
			...
			function addInput(focusme) {
				...
				holder.click(function() {
					if (options.input_min_size < 0 && feed.length) {
						if (options.json) {
							load_Json(xssPrevent(input.val(), 1))
							$("#defaultText").hide();
						} else {
				...
				function resizeInput(event) {
					...
					//auto expand input
					var newsize = (options.input_min_size > input.val().length) ? options.input_min_size : (input.val().length + 3);
				...
				input.keyup(function(event) {
					...
					if(etext==' ' && !$.trim(input.val())){//允许敲空格打开菜单，但不显示
						etext = '  ';//需要多余一个字符，否则load_Json函数不发起ajax请求
					}
			...
			function addMembers(etext, data) {
				...
				if (data != null && data.length) {
					$.each(data, function(i, val) {
						cache.set(xssPrevent(val.key), xssPrevent(String(val.value).split('human">').join('human" term="' + etext + '">')));
					});
				}
				...
				$.each(cache.search(etext), function(i, object) {
					if (options.filter_selected && element.children('option[value="' + object.key + '"]').hasClass("selected")) {
						...
						content += '<li rel="' + object.key + '">' + xssDisplay(object.value) + '</li>';
						counter++;
						maximum--;
					}
				});
			...
			function bindEvents() {
				var maininput = $("#" + elemid + "_annoninput").find(".maininput");
				bindFeedEvent();
					...
					if (options.cache && json_cache_object.get(etext)) {
						...
						setTimeout(function() {
							...
							$.getJSON(options.json_url, {
								"key": xssDisplay(etext)
							}, function(data) {
								if (!isactive)
									return;
								// prevents opening the selection again after the focus is already off
			...
			function filter(array, term) {
				...
				return $.grep(array, function(value) {
					return matcher.test(value.regex || value.value || value);
				});
			}

			function load_Json(etext) {
				counter = 0;
				if (options.json && maxItems()) {
					if (options.cache && json_cache_object.get(etext)) {
						addMembers(etext);
						bindEvents();
					} else {
						...
						setTimeout(function() {
							...
							if (!isactive)
								return;
							var o = eval(options.json);
							addMembers(etext, filter(o, xssDisplay(etext)));
							json_cache_object.set(etext, 1);
							bindEvents();
						}, options.delay);
					}
			...
			var element = $(this);
			var elemid = element.attr("id") || (randomId() + new Date().getTime());
			var getBoxTimeout = 0;
